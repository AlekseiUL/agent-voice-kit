...
After the publisher exists and the signed `v0.3.2` release is complete, run the GitHub workflow **Publish to PyPI** with input `v0.3.2`. The workflow checks that the package version matches the selected tag, builds in a separate low-privilege job, and gives `id-token: write` only to the publication job.
...
uv tool install agent-voice-kit==0.3.2
agent-voice --version
```

Also verify `https://pypi.org/project/agent-voice-kit/` shows version `0.3.2` and the expected project links. If publication fails, do not reuse or expose tokens; inspect the Trusted Publisher tuple above first.
