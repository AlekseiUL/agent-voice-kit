...
Готовый фрагмент попадает в checkpoint только после проверки. Повторный запуск с тем же текстом и настройками использует сохранённые части. Манифест содержит хеши и метаданные аудио, но не исходный текст. Логический срок хранения — семь дней; версия 0.1.0 пока не удаляет просроченные каталоги автоматически.
